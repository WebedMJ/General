<###########################################################################
 ...
 IMPORTANT: Must be run from an elevated command prompt on a Veeam Backup server.
            Also needs saved passwords for vSphere User and VM admin user.
 ...
 1.6 2018-05-23 - Modified time range to look for valid backups to account for longer jobs

 USAGE: .\VeeamFileRestore-auto.ps1 -TargetVM <VMName> -FilesToRestore <File> -Reason <VeeamJobReason> -BackupDate <ddmmyyyy>
...
(
    ...
    [Parameter(Position = 5, Mandatory = $false)]$EmailRecipient = "<EMAILADDRESS>",
    [Parameter(Position = 6, Mandatory = $false)]$RemoteVeeamSvr = "<VEAMSERVER>"
)
...
$vSphereUser = "DOMAIN\USER"
$vSpherePasswordfile = ".\Credentials\VSPHEREUSERPW.txt"
$GuestVMUser = "DOMAIN\USER"
$GuestVMPasswordfile = ".\Credentials\VMUSERPW.txt"
